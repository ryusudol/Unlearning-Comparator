...
async def evaluate_model(model, data_loader, criterion, device, is_training=False):
    ...
    class_accuracies = {i: (100 * class_correct[i].item() / class_total[i].item() if class_total[i].item() > 0 else 0) for i in range(10)}
    
    return total_loss / len(data_loader), accuracy, class_accuracies

async def unlearn_FT_model(model,
                           retain_loader,
                           train_loader,
                           test_loader,
                           criterion, 
...
async def run_unlearning_FT(request, status, weights_path):
    ...
    try:
        model = await unlearn_FT_model(model=model, 
                                       ...
                                       status=status,
                                       model_name="resnet18",
                                       dataset_name=f"CIFAR10_FT_forget_class_{request.forget_class}",
