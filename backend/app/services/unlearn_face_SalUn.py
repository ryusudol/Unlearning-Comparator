...
from app.threads import UnlearningFaceSalUnThread
...
async def unlearning_face_SalUn(request, status, base_weights_path):
    ...
    unlearning_SalUn_thread = UnlearningFaceSalUnThread(
        request=request,
        status=status,
...
async def run_unlearning_face_SalUn(request, status, base_weights_path):
    try:
        status.is_unlearning = True
        status.progress = "Unlearning"
        updated_status = await unlearning_face_SalUn(request, status, base_weights_path)
