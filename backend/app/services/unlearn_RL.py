...
from app.threads import UnlearningRLThread
...
async def unlearning_RL(request, status, base_weights_path):
    ...
    unlearning_RL_thread = UnlearningRLThread(
        request=request,
        status=status,
    ...
    del unlearning_RL_thread
    del model_after, optimizer, scheduler
    del train_loader, test_loader, train_set, test_set
    del retain_loader, retain_subset
    del forget_loader, forget_subset
    del criterion
