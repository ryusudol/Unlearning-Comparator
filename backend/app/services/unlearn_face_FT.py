...
from app.threads import UnlearningFaceFTThread
...
async def unlearning_face_FT(request, status, base_weights_path):
    ...
    unlearning_FT_thread = UnlearningFaceFTThread(
...
async def run_unlearning_face_FT(request, status, base_weights_path):
    try:
        status.is_unlearning = True
        status.progress = "Unlearning"
        updated_status = await unlearning_face_FT(request, status, base_weights_path)
