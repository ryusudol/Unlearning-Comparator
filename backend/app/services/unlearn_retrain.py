...
async def unlearning_retrain(request, status):
    ...
    unlearning_thread = UnlearningRetrainThread(
        model, unlearning_loader, train_loader, test_loader, criterion, optimizer, scheduler,
    ...
    if unlearning_thread.exception:
        print(f"An error occurred during Retrain unlearning: {str(unlearning_thread.exception)}")
        return status
