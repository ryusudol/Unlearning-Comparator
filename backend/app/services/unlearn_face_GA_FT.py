...
from app.threads import UnlearningFaceGAFTThread
...
async def unlearning_face_GA_FT(request, status, base_weights_path):
    ...
    unlearning_GA_FT_thread = UnlearningFaceGAFTThread(
        request=request,
        status=status,
...
async def run_unlearning_face_GA_FT(request, status, base_weights_path):
    try:
        status.is_unlearning = True
        status.progress = "Unlearning"
        updated_status = await unlearning_face_GA_FT(request, status, base_weights_path)
